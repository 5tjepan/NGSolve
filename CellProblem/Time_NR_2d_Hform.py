...
brid=0.16
core = MoveTo(-0.04,-0.08).Line(brid/2,0.0).Line(0,brid).Line(-brid/2,0).Close().Face()
...
mesh = Mesh(OCCGeometry(core, dim=2).GenerateMesh(maxh=0.01, quad_dominated=False))
print('mesh.nv',mesh.nv)
print('mesh.nedge', mesh.nedge)
print('mesh.ne', mesh.ne)

#.............
fes = H1(mesh, order=0, dirichlet="interface", complex=False)
H, v = fes.TnT()

omega=2*pi*1
dt = 2*pi/omega /100
cnt = 0; t0=0.0 ; time = t0; tend=1.7
interval=(0.4,1.0); samplestep=2

mu0 = 1.257e-6
#rel = 200
#sigma = 2e3
sigma = mesh.MaterialCF({ "core" : 2e3 }, default=0)
Kf=1
...
gfu.Set(bc, definedon=mesh.Boundaries('interface'))
...
#for iter in range(1,14):  
    ...
    for it in range(1, 8):  #NewtonRaphson petlja
        ...
        mud=(oldB-prevB)/(oldgfu-gfuPrev+1e-4)  # ALTERNATIVNA DEFINICIJA za mud koja se oslanja na vremensku derivaciju
