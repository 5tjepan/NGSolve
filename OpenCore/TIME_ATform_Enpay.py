...
fsU = HCurl(mesh, order=0, dirichlet="outer|outfront|outright", complex=False, nograds = False)#, gradientdomains = graddom)

#updateHCurlRegionOrder(fsU, 0, "core")
#------------

#fsU = HCurl(mesh, order=0, dirichlet="outer", complex=False, nograds = False)
fsV = H1(mesh, order=1, dirichlet='left|bot|top', definedon='core', complex=False) #bez right!
fes=fsU*fsV
...
omega=2*pi*50 #50Hz
dt = 2*pi/omega /51
cnt = 0; t0=0.0 ; time = t0; tend=1.1*(2*pi/omega)
...
prevApot, prevTpot = prev.components
...
Ts_of_t = sin(omega*t) #vremesnka ovisnost pobude Ts o vremenu
...
#    gfuD.Set(bc,definedon=mesh.Boundaries('interface')) #vrem. ovisan bc (t.Set(time))
    ...
    for i in range(1,8):
        ...
        term2= -dt*(rho*(rot*grad(csp)))*(rot*grad(tau))*dx('core') - (rot*grad(tau))*mvp*dx('core')
        ...
        prevterm= -(rot*grad(tau))*mvp*dx('core')
        prevmat=BilinearForm(prevterm)
        prevmat.Assemble()
        ...
        Ts_total = Ts_of_t*Ts_coil *curl(alpha)*dx("coil") + Ts_of_t*Ts_air *curl(alpha)*dx("air|core") 
        f=LinearForm(Ts_total)
        #f = LinearForm(fes)
        #f += Ts_of_t*Ts_coil *curl(alpha)*dx("coil") + Ts_of_t*Ts_air *curl(alpha)*dx("air|core") 
    ...
    Jlistoftuple.append((rot*grad(Tpot))(mesh(0.00498,0.01,0.098)))
    time_axis.append(time)

    #current power:
    #grad(gfu)*grad(gfu)==rot*grad(gfu)*rot*grad(gfu)
    currPow=Integrate((rho*(rot*grad(Tpot)))*(rot*grad(Tpot)), mesh, 
