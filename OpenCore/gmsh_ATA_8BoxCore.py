...
mesh_g = ReadGmsh("gmsh_BoxCore_eighth/Box_eighthH10k.msh")
...
fsU = HCurl(mesh, order=0, dirichlet="gamaB|gamaE", complex=True, nograds = False)#, gradientdomains = graddom)
#updateHCurlRegionOrder(fsU, 2, "core")
#####################

#fsU = HCurl(mesh, order=0, dirichlet="gamaB|gamaE", complex=True, nograds = False)
fsV = H1(mesh, order=1, dirichlet='topleft|gamaH', definedon='core', complex=True) #bez right!
fes=fsU*fsV
...
#air = mesh.MaterialCF({ "air_coil" : 1 }, default=None)
...
for i in range(1,2):
    ...
    term1=(1/mu0)*curl(mvp)*curl(alpha)*dx('air_coil') + rel*curl(mvp)*curl(alpha)*dx('core') \
    - (rot*grad(csp))*alpha*dx('core') #+ 0.1*mvp*alpha*dx('air_coil')
    ...
    Ts_total = Ts *curl(alpha) * dx("air_coil") #+ Ts_air *curl(alpha) * dx("air_coil") 
