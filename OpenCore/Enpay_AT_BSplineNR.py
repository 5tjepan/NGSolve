...
def MakeGeometry():
    ...
    back_outer = Plane (Pnt(dbox*2/3,0,0), Vec(1, 0,0) ).bc("outer")
    left_outer = Plane (Pnt(0,dbox*2/3,0), Vec(0,1,0) ).bc("outer")
    ...
    bottom  = Plane (Pnt(tocx,tocy,-0.1), Vec(0,0,-1) ).bc("bottom")
    back = Plane (Pnt(0.005,0.02,0.1), Vec(1, 0,0) ).bc("back").maxh(0.0025)
    left = Plane (Pnt(0.005,0.02,0.1), Vec(0,1,0) ).bc("left")
    top  = Plane (Pnt(0.005,0.02,0.1), Vec(0,0, 1) ).bc("top")
    core = left * right * front * back * bottom * top
...
ngmesh = MakeGeometry().GenerateMesh(maxh=0.07)
mesh = Mesh(ngmesh)
mesh.Curve(5)  
...
fsV = H1(mesh, order=1, dirichlet='left|bottom|top', definedon='core', complex=True) #bez right!
fes=fsU*fsV
