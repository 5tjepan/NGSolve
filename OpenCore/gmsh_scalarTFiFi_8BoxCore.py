...
fsU = H1(mesh, order=1, dirichlet="topleft|gamaH", definedon='core', complex=True) #bez right!
fsV = H1(mesh, order=1,  dirichlet="gamaH", complex=True) #bez right!
fes=fsU*fsV
...
for i in range(1,2):
    ...
    Ts_total = -1j*omega*(mu*Ts)*CF((1,0,0))*tau*dx('core') + 1j*omega*mu*Ts*grad(psi)*dx 
...
Jpost=rot*grad(Tpot)
Bpost=mu*(Tpot*CF((1,0,0)) - grad(Fpot) + Ts)


Draw (Bpost, mesh, "B", draw_surf=False)
Draw (Jpost, mesh, "J", draw_surf=False)
Draw (Tpot, mesh, "Tpot", draw_surf=False)
