...
fsV = H1(mesh, order=1, dirichlet='right', definedon='core', complex=False) #bez right!
fes=fsU*fsV
...
omega=2*pi*50 #50Hz
dt = 2*pi/omega /51
cnt = 0; t0=0.0 ; time = t0; tend=0.5*(2*pi/omega)
...
#    gfuD.Set(bc,definedon=mesh.Boundaries('interface')) #vrem. ovisan bc (t.Set(time))
    ...
    for i in range(1,8):
        ...
        term3= 1e0*mvp*alpha*dx('air|coil') + 1e0*(esp)*(phi)*dx('core')
        ...
        Ts_total = dt*Ts_of_t*Ts_coil *curl(alpha)*dx("coil") + dt*Ts_of_t*Ts_air *curl(alpha)*dx("air|core") 
    ...
    Jlistoftuple.append(sigma*(-Apot - grad(Vpot))(mesh(0.00498,0.01,0.098)))
    time_axis.append(time)

    #current power:
    #grad(gfu)*grad(gfu)==rot*grad(gfu)*rot*grad(gfu)
    currPow=Integrate(sigma*(-Apot - grad(Vpot))*(-Apot - grad(Vpot)), mesh, 
