...
brid=0.16
inner = MoveTo(-0.08,-0.08).Line(brid,0.0).Line(0,brid).Line(-brid,0).Close().Face()
...
def TimeStepping(invmstar, initial_cond = None, t0 = 0, tend = 2,
    ...
    while time < tend - 0.5 * dt:
        ...
        gfuPrev.vec.data=gfu.vec
        Blist.append(curl(gfu)[0](mesh(0,0.49*brid)))
        Jlistoftuple.append(DgfuDt(mesh(0,0.49*brid)))
...
Jlist=[-1*sigma*elem[0] for elem in Jlistoftuple] #elem[0] je Jx komponenta

plt.figure()

plt.subplot(211)
plt.plot(time_axis[1:],Blist[1:],'b')
plt.grid(True)
#...
plt.subplot(212)
plt.plot(time_axis[1:], Jlist[1:],'r--')
plt.grid(True)
plt.show()
