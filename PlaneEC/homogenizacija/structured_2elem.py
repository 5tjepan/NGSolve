...
term1 = rho*grad(csp)*grad(theta)*dx + 1j*omega*curl(mvp)*theta*dx # 1j*omega*mvp*(rot*grad(theta))*dx
...
term2 = - 1j*omega* rel*curl(mvp)*curl(alpha)*dx + 1j*omega*rot*grad(csp)*alpha*dx + 0.10*mvp*alpha*dx 
term2hg= 1/12*(omega*kh)**2 /rho *curl(mvp)*curl(alpha)*dx

term3a= (-1j*omega*kh/3*0 - kh**3*omega**2/(120*rel*rho)) *curl(mvp)*(rot*grad(theta)*diry)*dx
term3b= (-1j*omega*kh/3*0 - kh**3*omega**2/(120*rel*rho)) *curl(alpha)*(rot*grad(csp)*diry)*dx
...
print('ReactivePower =', Integrate(RePower,mesh,order=5).real)
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
