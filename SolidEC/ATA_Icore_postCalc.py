...
################################################## POST:::
core = mesh.Materials('core')

postfes = H1(mesh, order=1, definedon='core') #dirichlet=Svi_dirich
Blin = postfes.TrialFunction()
beta = postfes.TestFunction()

#posta = BilinearForm( ( Blin*beta + 5e-5*grad(Blin)*grad(beta))*dx).Assemble()
posta = BilinearForm( ( Blin*beta + 1e-4*grad(Blin)[1]*grad(beta)[1])*dx).Assemble()
postf = LinearForm(B.Norm()*beta*dx).Assemble()
...
coreVol= Integrate(CF(1), mesh, definedon=core)
print('area= ',coreVol)

B_avg= Integrate(B.Norm()/coreVol, mesh, definedon=core)
print('B_avg=',B_avg)

Bpost_avg= Integrate(Bpost.Norm()/coreVol, mesh, definedon=core)
print('Bpost_avg=',Bpost_avg)

#Bset_avg= Integrate(Bset.Norm()/coreVol, mesh, definedon=core)
#print('Bset_avg',Bset_avg)
...
#Jset = GridFunction(postfes)
#Jset.Set(J.Norm())
JH1 = GridFunction(postfes)
JH1.vec.data = Jposta.mat.Inverse(postfes.FreeDofs()) * Jpostf.vec

Draw(JH1, mesh, 'JH1')
#Draw(Bset, mesh, 'Bset')

J_avg= Integrate(J.Norm()/coreVol, mesh, definedon=core)
print('J_avg=',J_avg)

JH1_avg= Integrate(JH1.Norm()/coreVol, mesh, definedon=core)
print('JH1_avg=',JH1_avg)

#...........................
#Pow=0.5*rho_yz*(J[1]*Conj(J[1]) + J[2]*Conj(J[2]))
Pow=0.5*rho*J*Conj(J) # + 0.5* 1/12*(kh*omega)**2 /rho_yz *(B*xdir)*(Conj(B)*xdir)
Peddy=Integrate(Pow.real, mesh, order=5)
print('Pow=',Peddy) 

Pow_post=0.5*rho_yz*(JH1*JH1) # + 0.5* 1/12*(kh*omega)**2 /rho_yz *(B*xdir)*(Conj(B)*xdir)
Peddy_post=Integrate(Pow_post, mesh, order=5, definedon=core)
print('Pow_post=',Peddy_post)
#.......

Bsqr=Integrate((B.Norm()*B.Norm()), mesh, order=5, definedon=core)
print('B**2=',Bsqr)

Bpost_sqr=Integrate((Bpost*Bpost), mesh, order=5, definedon=core)
print('Bpost**2=',Bpost_sqr)
#.............................
