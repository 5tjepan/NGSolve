...
fsV = H1(mesh, order=1, dirichlet='outer', definedon="core", complex=True)
fes=fsU*fsV
...
term4=1j*1e0*mvp*alpha*dx('air|coil') + 1j*1e0*grad(esp)*alpha*dx('air|coil') + \
    1j*1e0*mvp*grad(phi)*dx('air|coil') + 1j*1e0*grad(esp)*grad(phi)*dx('air|coil')
